...
struct  IpPrinterBase {
	template<typename T>
	string print_ip_from_numeric(const T& ip) {
		return static_cast<Derived*>(this)->print_ip_from_numeric_impl(ip);
	}

	template<typename Arg, typename ...Args>
	string print_ip_from_tuple(const std::tuple<Arg, Args...>& ip) {
		return static_cast<Derived*>(this)->print_ip_from_tuple_impl(ip);
	}

	template<typename Container>
	string print_ip_from_container(const Container& collect, bool b_reverse = false) {
		return static_cast<Derived*>(this)->print_ip_from_container_impl(collect, b_reverse);
	}
};
...
class IpPrinter : public IpPrinterBase<IpPrinter> {
	...
	template<typename Container>
	string print_ip_from_container_impl(const Container& collect, bool b_reverse = false) {
	...
	template<typename T>
	string print_ip_from_numeric_impl(const T& ip) {
	...
	template<typename Arg, typename ...Args>
	string print_ip_from_tuple_impl(const std::tuple<Arg, Args...>& ip) {
...
string print_ip(const T& ip) {
	...
	if constexpr (is_integral_v<T>) {
		return ipPrinter.print_ip_from_numeric(ip);
	}
	else if constexpr (is_same_v<T, string>) {
		return ip;
	}
	else if constexpr (is_tuple_v<T>) {
		return ipPrinter.print_ip_from_tuple(ip);
	}
	else if constexpr (is_list_v<T> || is_vector_v<T>) {
		if constexpr (is_integral_v<typename T::value_type>) {
			return ipPrinter.print_ip_from_container(ip);
		}
	}
	return "";
}
